...
Streaming data processing differentiates itself from the classic data processing for its use of unbounded datasets, that is a continuous and endless data flow which needs of adequate abstractions in order to be able to apply operators or transformations like Map, Reduce and Filter operations. \\ 

Generally, there are 2 execution models usable when to approach an unbound dataset:
...
A Batch based execution is feasibly usable when requirements on the state management, in-order consuming and windowing are not present or very relaxed and, thus, a Streaming approach is generally favoured because of the conceptual paradigm which defines it: Dataflow Programming.

\paragraph{Dataflow Programming}  \label{DataflowProg} ~\\

Dataflow Programming is programming paradigm which models an application as a Direct Acyclic Graph (DAG) and thus it differentiates itself very heavily from the Imperative Programming (or Control Flow), which models a program like finite sequence of operations.\\
...
\begin{itemize}
	...
	\item \textbf{Table API}: it's a declarative DSL \footnote{Domain Specific Language: a language with a limited expressivity which focuses on a given domain of use.} which follows the extended relational model and offers the possibility to model a data stream like a table upon which it's possible execute operations like projections, aggregations and groupings. Despite being less expressive with respect to the Core Api, these APIs allow a greater compactness when it comes to describe the operations to be executed on the data.
	\item \textbf{SQL}: it's the highest level of abstraction offered by Flink and interacts directly with the Table APIs in order to provide a representation of the application being developed through SQL queries, executable on the tables defined by the Table API, directly on the data.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Sinks}, They are the application output, being it a file, a process or another queue. They are the ending nodes of the application DAG
\end{itemize}
...
Applications using Flink are inherently parallels and distributed. During execution, a stream may have more than one partition, while each operator can have more than a single sub-task operating on data, each one independent from the other and executed on different threads and, if possible, on different machines or containers.\\
The parallelism of a task is the parameter indicating the number of subtasks an operator can have and, consequentially, the number of the partitions of the stream getting outputted from that operator.\\
...
Streams can transport data between 2 operators following 2 different patterns: One-to-one pattern and redistributing pattern:
\begin{itemize}
    ...
    \item \textbf{Redistributing streams} change their partitioning, sending the data to different subtasks, based on the transformation being applied. For example, a keyBy() operator repartition the stream using the hash of the chosen key. In this case, the elements' order will be preserved only between neighbouring operations pairs, and won't be ever guaranteed being the same in the following operators.
\end{itemize}
...
In a Pattern there can be, analogously to RegExp, Quantifiers and Conditions.
...
Similarly to conditions combination, pattern combination is possible concatenating the calls of the following methods allowing to specify the contiguity constraints which a given pattern should satisfy:
...
Once a pattern sequence has been specified, it needs to applied to the target \verb|Datastream|, creating this way a \verb|PatternStream|. On this new stream needs to applied \verb|select| or \verb|flatSelect| operations in order to enable further processing on the events satisfying the pattern.

\subsubsection{Other extensions: Flink Gelly and FlinkML}

In addition to the aforementioned library for complex event handling, Flink makes available specific sets of API useful when it comes to enable advanced processing on the streaming data.

\paragraph{Flink Gelly} is an API based on the Flink \verb|Dataset|s API specifically developed for Graph Processing. Graphs are stored as a special case of  a \verb|Dataset|, a finite set of vertices and edges. This library allows the application of operators like Map, Filter, Intersect and Union, Mutations like edge and vertex removal, but also advanced techniques belonging to Graph Theory like \textit{Iterative Graph Processing} (Vertex-centric, Scatter-Gather and Gather-Sum-Apply), \textit{Single Source Shortest Paths}, \textit{Clustering} and Similarity analysis.
